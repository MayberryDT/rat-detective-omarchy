...
Version **1.3.2** keeps the compact live-scoreboard panel and adds opt-in
...
Dispatch status pages are capped at 64 KiB before QML receives them; oversized
...
That command installs the current **1.3.2** release. Existing installations can
...
the ten-rat cap. Overflow rooms still sleep. The 1.3.1 panel was verified against
the live service; 1.3.2 changes only the response-size boundary, which is covered
by local HTTP fixtures.
